...
class TUstudent(Human):
    ...
    def study_program(self, study_program):
        tu_programs = [
            "Aerospace Engineering", "Angewandte Linguistik", "Architektur",
            "Energy Science and Engineering", "Elektrotechnik und Informationstechnik"
        ]
    ...
    def courses(self, courses):
        if len(courses) != 5:
            raise ValueError("Minimum courses accomplished number should be 5")
    ...
    def favourite(self, favourite):
        if favourite not in self.courses:
            raise ValueError("Favourite courses have to be an accomplished course")
    ...
    def __str__(self):
        return (
            ...
            f"Age: {self._age}\n"
            f"Date: {self._date}\n"
            f"Study program: {self._study_program}\n"
            f"Matricule number: {self._matricule}\n"
            f"Accomplished courses: {self.courses}\n"
            f"Favourite courses: {self.favourite}"
        )


def main():
    s1 = TUstudent(
        "David Dupond", 21, "11-11-2011",
